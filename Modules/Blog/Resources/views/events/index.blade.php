                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title float-right"><a class="btn btn-info text-white"
                                        href="{{ route('events.create') }}"><i class="fa fa-plus"></i> Create</a> </h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                        ...
                                        @foreach ($events as $key => $value)
                                            <tr>
                                                ...
                                                <td class="text-center"><img
                                                        src="{{ asset('upload/images/events/' . $value->image) }}"
                                                        width="120px" alt="{{ $value->name }}"> </td>
                                                <td>{!! $value->shortdescription !!}</td>
                                                <td>{{ $value->date }}</td>
                                                <td class="text-center">
                                                    @if ($value->status == 'on')
                                                        <a href="{{ route('event.status', $value->id) }}"
                              ...
                              ">
                                                        ...
                                                        <form id="destroy{{ $value->id }}" class="d-none"
                                                            action="{{ route('events.destroy', $value->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('delete')
                                                        </form>
